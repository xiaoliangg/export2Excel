...
public class Links_ZhunerExport {

	public static void main(String[] args) throws Exception {
		String month = "2019-02";
		String partnerCode = "P000294";  //P000294  准儿翻译机  物联网卡
		
		DataPlan dataPlan = new DataPlan("DP20190121000402", "日本自选天数包（1G/日）");
		List<DataPlan> packList = new ArrayList<DataPlan>();
		packList.add(dataPlan);
		...
		export(partnerCode,packList,sups1,oper1,"0",month,exceptionMsg);//"0"表示不包含此运营商的话单 "1"表示包含此运营商的换单
		...
		//export(partnerCode,packList,sups2,oper2,"0",month,exceptionMsg);
		...
		//oper3.add(operator3);
		//export(partnerCode,packList,sups3,oper3,"0",month,exceptionMsg);
		
		
		//新版docomo统计,仅包含Docomo新（翻译机转旅游）   暂不需要
		//export(partnerCode,packList,sups3,oper3,"1",month,exceptionMsg);
	...
	private static void export(String partnerCode, List<DataPlan> packList, List<Supplier> suppList, List<Operator> operList, String includeOperators,
		...
		System.out.println("packageCodes:" + getPackageCodes(packList));
		...
		paramMap.put("IN_PACKAGE_CODE", getPackageCodes(packList));
		...
		countProc.countM2mcardZhunerProc(paramMap);
...
//        writePackage(path, packageFileName, fileType,details,resolvePackageOrderDaysDao,channel);
    ...
    private static String writeUsageAndPackage(String path, String fileName, String fileType, List<PackageUseDetails> details,
		...
		sheet.setColumnWidth(10, 256*22);
...
//				throw new Exception("根据套餐名无法确定天数");
	...
	private static String getPackageCodes(List<DataPlan> packList) {
		StringBuilder strB = new StringBuilder();
		for(DataPlan pack:packList){
			strB.append(pack.getPackageCode() + ",");
		}
		return strB.toString();
	}
	...
	private static String getPackageNames(List<DataPlan> packList) {
		StringBuilder strB = new StringBuilder();
		for(DataPlan pack:packList){
			strB.append(pack.getPackageName());
		}
		return strB.toString();
	}
