...
public class Links_ResolveExport {
	...
	public static void main(String[] args) throws Exception {
		...
		//export(sups1,oper1,"0",month,exceptionMsg);
		...
		//export(sups2,oper2,"0",month,exceptionMsg);
		...
		oper3.add(operator3);
		//export(sups3,oper3,"0",month,exceptionMsg);
		
		
		//新版docomo统计,仅包含Docomo新（翻译机转旅游）
		export(sups3,oper3,"1",month,exceptionMsg);
	...
	private static void export(List<Supplier> suppList, List<Operator> operList, String includeOperators,
		...
		countProc.countResolveProc(paramMap);
...
//        writePackage(path, packageFileName, fileType,details,resolvePackageOrderDaysDao,channel);
        ...
        channel = SysConstant.CHANNEL_HUIZHOU;
        List<PackageUseDetails> details = resolvePackageOrderDaysDao.getPackageUseDetails(channel);
        String fileName = month + "_resolve惠州套餐流量统计_" + getSupplerNames(suppList);
        ...
        channel = SysConstant.CHANNEL_SUZHOU;
        details = resolvePackageOrderDaysDao.getPackageUseDetails(channel);
        fileName = month + "_resolve苏州套餐流量统计_" + getSupplerNames(suppList);
        if(operList.size() > 0 && "1".equals(includeOperators)) {
        	fileName += "仅含" + getOperatorNames(operList);
        }else if(operList.size() > 0 && "0".equals(includeOperators)){
        	fileName += "不包含" + getOperatorNames(operList);
        }
        result = writeUsageAndPackage(path,fileName,fileType,details,resolvePackageOrderDaysDao,resolveExport3Dao,channel);
        exceptionMsg.add(fileName +"   "+  result);
        
        channel = SysConstant.CHANNEL_GANGTAI;
        details = resolvePackageOrderDaysDao.getPackageUseDetails(channel);
        fileName = month + "_resolve港台套餐流量统计_" + getSupplerNames(suppList);
        if(operList.size() > 0 && "1".equals(includeOperators)) {
        	fileName += "仅含" + getOperatorNames(operList);
        }else if(operList.size() > 0 && "0".equals(includeOperators)){
        	fileName += "不包含" + getOperatorNames(operList);
